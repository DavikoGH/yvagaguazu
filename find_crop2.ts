...
function findMatch(bg, fg, name) {
  ...
  console.log(`Finding match for ${name}: bg(${bgW}x${bgH}) fg(${fgW}x${fgH})`);
  
  // pick a few non-transparent pixels to use as fingerprint
  const samples = [];
  for (let y = 0; y < fgH; y += 15) {
    for (let x = 0; x < fgW; x += 15) {
      const idx = (fg.width * y + x) * 4;
      const a = fg.data[idx + 3];
      if (a > 200) { // fully opaque
        samples.push({
          x, y,
          r: fg.data[idx],
          g: fg.data[idx+1],
  ...
  const chosenSamples = samples.sort(() => 0.5 - Math.random()).slice(0, 15);
  
  let bestX = 0, bestY = 0;
  let minDiff = Infinity;
  
  for (let y = 0; y <= bgH - fgH; y += 5) {
    for (let x = 0; x <= bgW - fgW; x += 5) {
      let diff = 0;
  ...
  // Refine search around bestX, bestY
  ...
  console.log(`Match for ${name}: x=${bestX}, y=${bestY}, diff=${minDiff}`);
}
